...
void UEMSAsyncLoadGame::DeferredLoadActors()
{
	...
	//Load up to BatchSize actors (or until we run out)
	while (LoadedThisTick < BatchSize && LoadedActorNum < SavedActors.Num())
	{
		EMS->SpawnOrUpdateLevelActor(SavedActors[LoadedActorNum]);
		++LoadedThisTick;
		++LoadedActorNum;
	}

	//If we’ve processed the entire list, wrap up.
